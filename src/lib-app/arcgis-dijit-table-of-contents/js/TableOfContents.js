...
function (
    ...
    var Widget = declare([_WidgetBase, _TemplatedMixin, Evented], {
        ...
        _getLayerVisibility: function(layerIndex) {
            if (this.layers && this.layers.length) {
                ...
                if (featureCollection) {
                    ...
                    if (visibleLayers && visibleLayers.length)
                    	return featureCollection.layers[visibleLayers[0]].layerObject.visible;
                    else
                    	return featureCollection.layers[0].layerObject.visible;
                }
                else if(layerObject) {
                   return layer.layerObject.visible;
                }
            }
        },
        _toggleLayer: function(layerIndex, forceVisibility) {
            // all layers
            if (this.layers && this.layers.length) {
                var newVis;
                var layer = this.layers[layerIndex];
                ...
                if (featureCollection) {
                    ...
                    if (visibleLayers && visibleLayers.length) {
                        ...
                        newVis = forceVisibility !== undefined ? forceVisibility : newVis;
        ...
        _checkboxEvent: function(index) {
        	var onClickCallback = function(evt)
        	{
        		...
        		// Ctrl key modifier -> change all layer visibility to the opposite of the clicked layer
        		else {
        			var layers = this.get("layers");
        			var layerVis = this._getLayerVisibility(index);
        			        			
                    if (layers && layers.length) {
                        for (var i = 0; i < layers.length; i++) {
                        	this._toggleLayer(i, ! layerVis);
                        }
                    }
                    event.stop(evt);
        		}
            ...
            var checkEvent = on(this._nodes[index].checkbox, 'click', lang.hitch(this, onClickCallback));
            this._checkEvents.push(checkEvent);
            // when title is clicked
            var titleEvent = on(this._nodes[index].titleText, 'click', lang.hitch(this, onClickCallback));
